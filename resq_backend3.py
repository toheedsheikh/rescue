...
FLASK_SERVER_URL = "http://your-flask-server-url/predict"
...
def upload_report(
    ...
    try:
        ...
        # Send image and description to Flask server for predictions
        with open(image_path, "rb") as img_file:
            response = requests.post(FLASK_SERVER_URL, files={"file": img_file}, data={"description": description})

        if response.status_code != 200:
            raise HTTPException(status_code=500, detail=f"Failed to get prediction: {response.json()}")
        ...
        return {"message": "Report uploaded successfully", "report": new_row, "prediction": prediction}
...
def update_report_status(report_id: int, status: str = Form(...)):
    try:
        if status not in ['not_resolved', 'resolved', 'in_progress']:
